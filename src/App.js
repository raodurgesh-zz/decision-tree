...
export default class Tree extends Component {
  ...
  render() {
    ...
    const InteractionInput =({node, path})=> (<InterationBox type="input" showRemoveButton={this.state.treeData[0] && this.state.treeData[0].title} removeNode={() => this.removeNode({ path })} setTitle={(title) => this.changeTreeNode({ node, path, title, type: 'node' })} />)
    const InteractionDropdown = ({node , path}) => (<InterationBox type="ddl" options = { this.getEachNodeTitle()} showRemoveButton={true}  removeNode={() => this.removeNode({ path })} setTitle={(title) => this.changeTreeNode({ node, path, title, type: 'node' })} />)
    ...
    const renderNode = ({ node, path }) => {
      ...
      switch (type) {
        case 'interaction-input': return InteractionInput({node, path});
        case 'interaction-ddl': return InteractionDropdown({node, path});
        default:
          return (
            <div className="rst__rowTitle__inner">{node.title}</div>)
      }
    }
    ...
    const showInteractionMenu = ({ event, node, path }) => {
      ...
      this.setState({ interationMenu: { show: true, coordinate: { left, top }, default: { node, path } } })
    }
    ...
    return (
      ...
      <div>
        {interationMenu.show && <InterationMenu coordinate={interationMenu.coordinate} hideInteractionMenu={hideInteractionMenu} selectedItem={selectedInteraction} />}
            ...
            generateNodeProps={({ node, path }) => ({
              ...
              buttons: (() => {
                return node.type === 'node' ?
                  ([
                    <div className="rst__rowNodeAdd"
                      onClick={(event) =>
                        showInteractionMenu({ node, path, event })
                      }
                    >
                    </div>,
                    <div className="rst__rowNodeDelete"
                      onClick={() =>
